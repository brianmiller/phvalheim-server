...
function whatsNewNotes() {
    return [
        '2.47' => [
            ...
            'Fixed: <b>Check Now took half a minute and gave no sign it was working.</b> Nearly all of that is steamcmd signing in to Steam, and it was being paid again for every world and every click. The published build is the same for every world, so it is now fetched once and reused for 15 minutes &mdash; a check that reuses it finishes in under half a second instead of 32. The check also runs in the background now, with a spinner, an explanation of what it is waiting for, and the buttons disabled until it finishes. <b>Re-check Steam</b> next to it forces a real fetch when the cached answer is the thing you doubt.',
