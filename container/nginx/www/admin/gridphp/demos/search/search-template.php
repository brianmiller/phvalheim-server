...
// Code to identify filtered results
$grid["loadComplete"] = "function(){ 
	...
	if (grid[0].p.postData._search == true)
	{
		grid.jqGrid('setCaption', grid[0].p._caption + ' (Filtered)');
		$('span.ui-icon.ui-icon-search','.ui-pg-button').css('color','#D93025');
	}
	else
	{
...
$grid["search_options"]["tmplFilters"] = array(
	array(
		...
		"rules" => array (
						...
						array("field"=>"invdate", "op"=>"bw", "data"=>"{\"start\":\"2020-10-08\",\"end\":\"2020-11-25\"}"),		
						)
	),
	array(
		"groupOp" => "AND",
