		...
		<select name="tables" onchange="get_fields();" style="width:200px;">
			<?php
				...
				{ 
					// for postgres
					if ($db_conf["type"] == "postgres")
						$tabname = $rs["tablename"];
					else
						$tabname = $rs[0];

					if (!empty($restricted_tables) && in_array($tabname,$restricted_tables))
						continue;

					if (!empty($allowed_tables) && !in_array($tabname,$allowed_tables))
						continue;
						
					$sel = (($tabname == $_POST["tables"])?"selected":"");
				?>
					<option <?php echo $sel?>><?php echo $tabname?></option>
				<?php
				}
			?>
		</select>
