...
- **Fixed world card layout: a clipped hint, and Launch! floating away from the world name.** Both came from the card tables being 100% height. A card stretches to match the tallest card in its row, and a full-height table absorbs that slack by spreading its own rows — so the gap under the world name varied from card to card (82px, 55px and 17px on one page), and the crossplay hint was pushed past the bottom of the card and cut off.
