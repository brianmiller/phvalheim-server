...
- **"Use Access List" now fails closed.** Valheim enforces `permittedlist.txt` only when it has entries, so switching the access list on and leaving it empty produced an open server. When the list is enforced and the operator has added nobody, PhValheim now writes a single placeholder entry — Steam account ID 0, which is never issued to anyone — so the switch means what it says. It is injected when the file is rendered and never stored, so the database and the admin UI still show exactly what the operator typed, and it is removed the moment a real player is added or the access list is switched off. Because the file is re-rendered at every world start, this also covers the paths the save-time guard cannot see: create, clone and restore.
...
- **Players can find their own Steam ID.** It now appears under the avatar on the player page in small magenta, click to copy — the value a server owner has to ask every player for, which previously required a third-party lookup site.
