...
ms.time.sleep = lambda s: real_sleep(0.01)        # fast-forward the wait loop
...
take_at = src.index("lock = take_global_lock(args.trigger)")
loop_at = src.index("for s in todo:", take_at)
check("main() takes the lock BEFORE the source loop", True, take_at < loop_at)
...
orph_at = src.index("AND NOT EXISTS (SELECT 1 FROM mod_deps d WHERE d.version_id = v.id)")
moved_at = src.index("if catalogue_moved:")
want_at = src.index("want = sorted(set(ids)")
check("zero-edge versions are collected for re-resolution", True, orph_at > 0)
check("...UNCONDITIONALLY, not only when the catalogue moved", True, orph_at > moved_at)
